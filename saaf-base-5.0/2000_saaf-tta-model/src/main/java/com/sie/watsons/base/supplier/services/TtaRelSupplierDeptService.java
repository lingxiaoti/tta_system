...
@RestController
@RequestMapping("/ttaRelSupplierDeptService")
public class TtaRelSupplierDeptService extends CommonAbstractService{
	private static final Logger LOGGER = LoggerFactory.getLogger(TtaRelSupplierDeptService.class);

	@Autowired
	private ITtaRelSupplierDept ttaRelSupplierDeptServer;

	@Override
	public IBaseCommon getBaseCommonServer(){
		return this.ttaRelSupplierDeptServer;
	}
	...
	public String find(@RequestParam(required = false) String params,
		...
		try {
			...
			Pagination<TtaRelSupplierDeptEntity_HI_RO> result = ttaRelSupplierDeptServer.find(jsonObject, pageIndex, pageRows);
	...
	public String saveOrUpdate(@RequestParam(required = true) String params) {
		try {
			...
			TtaRelSupplierDeptEntity_HI instance = ttaRelSupplierDeptServer.saveOrUpdate(jsonObject, userId);
	...
	public String delete(@RequestParam(required = false) String params) {
		try {
			...
			for (String pkId : ids) {
				ttaRelSupplierDeptServer.delete(Integer.parseInt(pkId));
			}
	...
	public String findById(String params) {
		try{
			JSONObject jsonObject = JSON.parseObject(params);
			TtaRelSupplierDeptEntity_HI_RO instance = ttaRelSupplierDeptServer.findByRoId(jsonObject);
			return SToolUtils.convertResultJSONObj("S", SUCCESS_MSG, 0, instance).toString();
