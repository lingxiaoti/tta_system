...
public class TtaRelSupplierEntity_HI_RO {
	...
	public static final String TTA_REL_SUPPLIER_EXIST = "                  SELECT listagg('当前已有'||ts.supplier_code,'-') within group(order by trs.rel_supplier_id) valueName FROM tta_rel_supplier trs,\n" +
			"                                tta_supplier ts\n" +
			"                                where \n" +
			"                                trs.rel_id = ts.supplier_id\n" +
			"                                and trs.rel_supplier_code =:relSupplierCode \n" +
			"                                and trs.rel_id =:relId\n" +
			"                  union all\n" +
			"                  SELECT listagg('当前已有'||ts.supplier_code,'-') within group(order by trs.rel_supplier_id) valueName FROM tta_rel_supplier trs,\n" +
			"                                tta_supplier ts\n" +
			"                                where \n" +
			"                                trs.rel_id = ts.supplier_id\n" +
			"                                and trs.rel_supplier_code =:relSupplierCode \n" +
			"                                and trs.rel_id !=:relId\n" +
			"                                and exists (SELECT 1 FROM tta_supplier tss where tss.supplier_id =:relId and nvl(tss.is_latent,'N') = nvl(ts.is_latent,'N'))";
