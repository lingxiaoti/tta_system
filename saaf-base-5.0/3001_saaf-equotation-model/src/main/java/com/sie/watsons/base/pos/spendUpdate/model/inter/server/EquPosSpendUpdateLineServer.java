...
import com.sie.watsons.base.pos.spendUpdate.model.entities.readonly.EquPosSpendUpdateLineEntity_HI_RO;
...
@Component("equPosSpendUpdateLindServer")
public class EquPosSpendUpdateLineServer extends BaseCommonServer<EquPosSpendUpdateLineEntity_HI> implements IEquPosSpendUpdateLine {
    private static final Logger LOGGER = LoggerFactory.getLogger(EquPosSpendUpdateLineServer.class);

    @Autowired
    private ViewObject<EquPosSpendUpdateLineEntity_HI> equPosSpendUpdateLineDAO_HI;
    @Autowired
    private BaseViewObject<EquPosSpendUpdateLineEntity_HI_RO> equPosSpendUpdateLineDAO_HI_RO;
    @Autowired
    private EquPosSupplierInfoDAO_HI supplierInfoDao;

    public EquPosSpendUpdateLineServer() {
        super();
    }

    @Override
    public JSONObject findSpendUpdateLinePagination(JSONObject jsonObject, Integer pageIndex, Integer pageRows) {
        StringBuffer sqlBuffer = new StringBuffer(EquPosSpendUpdateLineEntity_HI_RO.QUERY_LINE_SQL);
...
//        jsonObject.put("updateHeadId", jsonObject.getInteger("updateHeadId"));
//        SaafToolUtils.parperParam(jsonObject, "t.update_head_id", "updateHeadId", sqlBuffer, map, "=");
        // 排序
        sqlBuffer.append(" order by t.last_update_date desc");
        Pagination<EquPosSpendUpdateLineEntity_HI_RO> pagination = equPosSpendUpdateLineDAO_HI_RO.findPagination(sqlBuffer, map, pageIndex, pageRows);
        return JSONObject.parseObject(JSONObject.toJSONString(pagination));
    }

    @Override
    public void saveSpendUpdateLineList(Integer updateHeadId, List<EquPosSpendUpdateLineEntity_HI> lineData, Integer userId) throws Exception {
        ...
        for (int i = 0; i < lineData.size(); i++) {
            EquPosSpendUpdateLineEntity_HI entityHi = lineData.get(i);
            for (int n = i + 1; n < lineData.size(); n++) {
                EquPosSpendUpdateLineEntity_HI entity2 = lineData.get(n);
            ...
            if (entityHi.getScore() != null) {
                if (entityHi.getScore().compareTo(BigDecimal.ZERO) < 0 || entityHi.getScore().compareTo(new BigDecimal("3")) > 0) {
                    throw new Exception("分数需要是大于等于0.0小于等于3.0的数字,修改后请重新保存");
                }
            }

            if (entityHi.getUpdateLineId() != null) {
                // 将有id的对象从游离态变成持久态,直接触发更新操作
                EquPosSpendUpdateLineEntity_HI entityHiById = equPosSpendUpdateLineDAO_HI.getById(entityHi.getUpdateLineId());
    ...
    @Override
    public int saveImportForSpendUpdate(String params, Integer userId) throws Exception {
...
//                }
                ...
                if (score != null) {
                    if (score.compareTo(BigDecimal.ZERO) < 0 || score.compareTo(new BigDecimal("3")) > 0) {
                        msgStr += "分数需要是大于等于0.0小于等于3.0的数字";
                    }
                }
                // 校验导入数据是否有重复供应商名称
    ...
    @Override
    public String deleteSpendUpdateLine(JSONObject jsonObject, int userId) {

        Integer updateLineId = jsonObject.getInteger("updateLineId");
        EquPosSpendUpdateLineEntity_HI entity = equPosSpendUpdateLineDAO_HI.getById(updateLineId);
        if (!ObjectUtils.isEmpty(entity)) {
            equPosSpendUpdateLineDAO_HI.delete(entity);
