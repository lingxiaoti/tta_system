...
@Component("baseAppAuthExcludeServer")
public class BaseAppAuthExcludeServer extends BaseCommonServer<BaseAppAuthExcludeEntity_HI> implements IBaseAppAuthExclude {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(BaseAppAuthExcludeServer.class);
	
	@Autowired
	private ViewObject<BaseAppAuthExcludeEntity_HI> baseAppWapAuthorizationExcludeDAO_HI;
	
	@Autowired
	private IBaseDeployeeAppInfo deployeeAppInfoServer;
	
	public BaseAppAuthExcludeServer() {
		super();
	}
	 ...
	 */
	@Override
	public List<BaseAppAuthExcludeEntity_HI> findByAppWapId(int appWapId){
		Map<String, Object> properties = new HashMap<String, Object>();
		properties.put("appWapId", appWapId);
		properties.put("deleteFlag", 0);
		return baseAppWapAuthorizationExcludeDAO_HI.findByProperty(properties);
	}
	...
	public JSONObject findJSONByAppWapId(int appWapId){
		...
		List<BaseAppAuthExcludeEntity_HI> instances = findByAppWapId(appWapId);
		if(instances != null && !instances.isEmpty()) {
			...
			for(BaseAppAuthExcludeEntity_HI instance : instances) {
				//部门
	 ...
	 */
	@Override
	public List<BaseAppAuthExcludeEntity_HI> findByAppWapIdAndType(int appWapId, int ouId, String objectType){
		Map<String, Object> properties = new HashMap<String, Object>();
		properties.put("appWapId", appWapId);
	...
	public void save(JSONObject paramsJSON, int userId) {
		...
		List<BaseAppAuthExcludeEntity_HI> instances = findByAppWapIdAndType(appWapId, ouId,objectType);
		//先保存部门
		...
		if(instances != null && !instances.isEmpty()) {
			for(BaseAppAuthExcludeEntity_HI instance : instances) {
				baseAppWapAuthorizationExcludeDAO_HI.delete(instance);
			}
		}
		LOGGER.info("save BaseAppAuthExcludeEntity_HI:{}", paramsJSON);
	}
	 ...
	 * @param userId 操作人ID
	 */
	private void saveByObjectType(int appWapId, String objectType, int ouId, boolean department, boolean isStoreDealer, JSONArray valuesJSON, List<BaseAppAuthExcludeEntity_HI> instances, int userId) {
		...
		for(int i=0; i<valuesJSON.size(); i++) {
			...
			BaseAppAuthExcludeEntity_HI instance = null;
			if(instances != null && !instances.isEmpty()) {
				...
				instance = new BaseAppAuthExcludeEntity_HI();
				//新增的需赋值应用ID、对象类型
			...
			baseAppWapAuthorizationExcludeDAO_HI.save(instance);
		}
	}
