CREATE OR REPLACE PACKAGE "PKG_TTA_PROPOSAL" IS
 ...
 PROCEDURE P_TTA_CONTRACT_GEN(P_ORDER_ID    NUMBER,
  ...
  PROCEDURE P_TTA_CLAUSE_COPY  (P_ORDER_ID    NUMBER,
    ...
    --条款框架复制
  PROCEDURE P_TTA_CLAUSE_COPY2  (P_ORDER_ID    NUMBER,
                                 P_USER_ID     NUMBER,
                                 P_ISSUCCESS   OUT NUMBER, --返回状态 -1：处理失败，1：成功；0：警告
                                 P_ERR_MESSAGE OUT VARCHAR2, --返回信息
                                 P_ID OUT  NUMBER
...
CREATE OR REPLACE PACKAGE BODY PKG_TTA_PROPOSAL IS
  ...
  BEGIN
    ...
    BEGIN
      ...
      for temph in (select t1.*
        ,t3.line_code as PARENT_LINE_CODE  from TTA_FEE_DEPT_LINE   T1, TTA_FEE_DEPT_LINE   T3 where
          t1.PARENT_LINE_ID = T3.FEEDEPT_LINE_ID(+)  and  t1.feedept_id = V_FEEDEPT_ID
  ...
  BEGIN
    ...
    BEGIN
      ...
      select count(1) into p_ttfhh_count  from TTA_TERMS_FRAME_HEADER_h ttfhh where ttfhh.year =p_year and ttfhh.business_version = p_next_business_version;

      if nvl(p_ttfhh_count,0)>0 then
            P_ISSUCCESS := -1;
            P_ERR_MESSAGE := '--条款名目已经存在相同年份,相同版本的数据';
            return ;
      end if ;
  ...
  BEGIN
    ...
    BEGIN
      select count(1) into P_COUNT_TTFH from   tta_terms_frame_header ttfh where ttfh.year =1800;
      IF NVL(P_COUNT_TTFH,0)>0 THEN
            P_ISSUCCESS := -1;
            P_ERR_MESSAGE := '--复制的时候默认年份是1800,已经存在1800年的单据';
            return ;
      END IF ;
    ...
    BEGIN
     ...
     --更新头表
      ...
      SET
       ...
       for  v in (select * from tta_clause_tree_h tcth where tcth.is_change =0
