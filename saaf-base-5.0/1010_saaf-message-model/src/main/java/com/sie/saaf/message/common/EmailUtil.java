...
//    }
     ...
     * * @throws Exception
     */
    public static MimeMessage createSimpleMail(Session session, MsgInstanceEntity_HI instance, MsgSourceCfgEntity_HI sourceCfg,JSONObject configJson)
            throws Exception {
        //创建邮件对象
       ...
       // message.setRecipient(Message.RecipientType.TO, new InternetAddress(instance.getReceiveCode()));
        message.setRecipients(Message.RecipientType.TO,InternetAddress.parse(instance.getReceiveCode()));
        //邮件的标题
        message.setSubject(instance.getMsgSubject());
        //邮件的文本内容
        message.setContent(instance.getMsgContent(), "text/html;charset=UTF-8");
        //返回创建好的邮件对象
        return message;
    }

    public static JSONObject sendMail(MsgInstanceEntity_HI instance, MsgSourceCfgEntity_HI sourceCfg) {
        ...
        try {
            ...
            Transport ts = session.getTransport();
            //3、使用邮箱的用户名和密码连上邮件服务器，发送邮件时，发件人需要提交邮箱的用户名和密码给smtp服务器，用户名和密码都通过验证之后才能够正常发送邮件给收件人。
            ts.connect(configJson.getString("ServerHost"), sourceCfg.getSourceUser(), sourceCfg.getSourcePwd());
            //4、创建邮件
