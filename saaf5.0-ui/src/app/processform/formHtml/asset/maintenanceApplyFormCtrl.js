...
define(['app','angularFileUpload','webconfig'], function (app,angularFileUpload,webconfig) {
    ...
    app.controller('maintenanceApplyFormCtrl', function ($scope, $location, $rootScope, $state, $stateParams, SIEJS,httpServer,URLAPI) {
        ...
        $scope.submitEvent.cusValidator = function(){
            ...
            if($scope.formParams.items.length < 1){
                SIEJS.alert('请填写报修详情信息','error','确定');
                return;
            }
            return true
        } 
        ...
        $scope.setRowInput = function (key,value,item) {
            ...
            for(var i=0;i<$scope.formParams.items.length;i++){
                ...
                if(data.assetId==item[0].assetId&&data.assetId!=-1){
        ...
        /*if ($scope.action == 'new') {
            $scope.createRows('items',{assetId:-1});
        }*/
        //审批流更新表单更换接口
        $(function () {
