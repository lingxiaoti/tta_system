...
define(['app','moment','angularFileUpload','../../../ems/service/http','webconfig'], function (app,moment,angularFileUpload,http,webconfig) {
    ...
    app.controller('markMedApplyFormCtrl', function ($scope, $location, $rootScope, $state, $stateParams, SIEJS,httpServer,URLAPI,pageResp,limitNumLength,$timeout) {
        $scope.params = {};
        $scope.storeParams = {};
        ...
        if($scope.action == 'new'){
            setTimeout(function(){
                $scope.formParams.itemMedia = [];
                $scope.formParams.itemMedia[0] = {};
        ...
        setTimeout(function () {
            httpServer.post(URLAPI.ems_findExpensePeriodByOrgList, {
                params: JSON.stringify({orgId: $scope.newParams.orgId, periodType: 'P',isFilter:'Y'}),
                pageIndex: 1,
                pageRows: 10000
            }, function (res) {
                if (res.status === 'S') {
