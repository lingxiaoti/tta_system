...
define(['app','moment'], function (app,moment) {
    app.controller('marketActivityFeeApplyFormCtrl', function ($scope, $location, $rootScope, $state, $stateParams, SIEJS,httpServer,URLAPI) {
        $scope.params = {};
        //费用期间参数+费用科目参数
        $scope.newParams = {};
        ...
        if($scope.action == 'new'){
            setTimeout(function(){
                ...
                //默认非外聘
                $scope.formParams.itemCompain[0].outHire = 'N';
                var p = {
        ...
        $scope.submitEvent.cusValidator = function(){
            //提交金额与费用合计一致
            $scope.formParams.encumbranceAmount = Number($scope.formParams.secItemCompainTotal[0].itemAmountTotal);
            // console.log($scope.formParams.encumbranceAmount)
            return true
        }
        //保存前回调
        ...
        $(function () {
            if($scope.action == "approval"){
                $("#processForm").attr("data-api-update","updateOaMarketingCostCompainByApproval");
            }
        });
        /*//流程审批，附件上传接口
