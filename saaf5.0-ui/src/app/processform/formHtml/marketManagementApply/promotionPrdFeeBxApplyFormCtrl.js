...
define(['app','moment','angularFileUpload','../../../ems/service/http','webconfig'], function (app,moment,angularFileUpload,http,webconfig) {
    ...
    app.controller('promotionPrdFeeBxApplyFormCtrl', function ($scope, $location, $rootScope, $state, $stateParams, SIEJS,httpServer,URLAPI,pageResp,limitNumLength,$timeout,iframeTabAction,processGetStartUrl) {
        $scope.params = {};
        $scope.storeParams = {};
        $scope.customerParams = {};
        $scope.expenseTypeSwitch=true
        $scope.expenseSwitch=false;
        //费用期间参数+费用科目参数
        $scope.newParams = {};
        $scope.tableEndit={};
        ...
        $scope.pageView.getBusinessFormCallback = function(res){
            if (res.status === 'S') {
                ...
                setTimeout(function(){
                    ...
                    $scope.formParams.departmentId = data.departmentCode;
        ...
        $scope.draftEvent.cusValidator = function(){
            ...
            var commitAmount=$scope.formParams.commitAmount;
            if(thisAmountTotal!=commitAmount){
                SIEJS.alert('本次付款金额不等于提交金额，无法保存', 'error', '确定');
                return false
            }
            $scope.getFileIds()
            console.log($scope.formParams)
        ...
        $scope.dealerHeaderSelectFun = function () {
            ...
            $scope.formParams.departmentCode = $scope.dealerModel_h.departmentId;
        ...
        $scope.expenseCallback= function () {
            console.log($scope.expenseSwitch)
            if(!$scope.expenseSwitch){
                return;
            }
            $scope.cleanRowData()
        ...
        $scope.contractAnountCallback=function(index,item){
            var obj=$scope.formParams.itemPromotion[index];
            if(obj.itemPrice&&obj.itemQuantity&&obj.otherAmount){
                var number=''
                number=Number(obj.itemPrice)*Number(obj.itemQuantity)+Number(obj.otherAmount);
                number = Math.floor(number * 100) / 100
                obj.contractAnount=number
                $scope.tableEndit.totalStrfn('thisAmount')
                $scope.tableEndit.totalStrfn('contractAnount')
            }
        }
        /*
         * 跳转
        ...
        $scope.detailTwo = function() {
            $scope.gotoProcessForm($scope.formParams,$scope.formParams.bcRemiburseId,'查看报销（不需申请:'+$scope.formParams.bcRemiburseId+'）','detail','N');
        }
        /*
         * 费用申请编号查看
         *
