 ...
 */
'use strict';
define(['app','moment','angularFileUpload'], function (app,moment,angularFileUpload) {
    app.useModule('angularFileUpload');//按需加载模块
    app.controller('productVerificationApplyFormCtrl', function ($scope, $location, $rootScope, $state, $stateParams, SIEJS,httpServer,URLAPI) {
        $scope.params = {};
        //费用期间参数+费用科目参数
        $scope.newParams = {};
        ...
        $scope.getStoreByPerson = function(key,value,item){
            $scope.formParams.items[0].storeName = item[0].storeName;
            $scope.formParams.items[0].storeCode = item[0].storeCode;
        }
        //经销商下属门店
        ...
        $scope.citySave1=function(){
            ...
            $scope.formParams.items[0][str]=data.cityName;
            $scope.formParams.items[0][RegionId]=data.id;
            $scope.formParams.items[0][country]=data.id[0];
            $scope.formParams.items[0][province]=data.id[1];
            $scope.formParams.items[0][city]=data.id[2];
            $scope.formParams.items[0][area]=data.id[3];
            $scope.formParams.items[0][country + 'Str']=data.name[0];
            $scope.formParams.items[0][province + 'Str']=data.name[1];
            $scope.formParams.items[0][city + 'Str']=data.name[2];
            $scope.formParams.items[0][area + 'Str']=data.name[3];
        ...
        $scope.getCustAccount3 = function(key,value,item){
            $scope.formParams.thiItems[$scope.rowIndex].dealerCode = item[0].customerId;
            $scope.formParams.thiItems[$scope.rowIndex].dealerName = item[0].customerName;
        }
        //费用期间
        ...
        if($scope.action == 'new'){
            setTimeout(function(){
                $scope.formParams.item = [];
                $scope.formParams.item[0] = {};
                $scope.formParams.items = [];
                $scope.formParams.items[0] = {};
                ...
                //默认非外聘
                $scope.formParams.items[0].outHire = 'N';
        ...
        $scope.submitEvent.cusValidator = function(){
            //提交金额 =产品核销明细合计金额+产品核销上帐明细合计金额
            $scope.formParams.encumbranceAmount = (Number($scope.formParams.secItemsTotal[0].amountTotal)*100 + Number($scope.formParams.thiItemsTotal[0].amountTotal)*100)/100;
            return true
        }
        //保存前回调
        ...
        $(function () {
            if($scope.action == "approval"){
                $("#processForm").attr("data-api-update","updateCampaignWriteoffProductByApproval");
            }
        });
