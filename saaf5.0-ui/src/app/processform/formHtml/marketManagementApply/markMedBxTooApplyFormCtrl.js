...
define(['app','moment','angularFileUpload','../../../ems/service/http','webconfig'], function (app,moment,angularFileUpload,http,webconfig) {
    ...
    app.controller('markMedBxTooApplyFormCtrl', function ($scope, $location, $rootScope, $state, $stateParams, SIEJS,httpServer,URLAPI,pageResp,limitNumLength,$timeout) {
        $scope.params = {};
        $scope.storeParams = {};
        ...
        $scope.getBuData = function (hasIds) {
            ...
            httpServer.post(URLAPI.queryLookupLineDic,{params: JSON.stringify({lookupType:'OA_PUT_AIM',systemCode:'OA'})},function(res){
                if (res.status === 'S') {
        ...
        $scope.checkOU = function (event, item) {
            ...
            $scope.formParams.itemMedia[0].deliveryPurpose = str;
            ...
            $scope.formParams.itemMedia[0].deliveryPurposeName  = str1;
        ...
        $scope.getCostUser = function(key,value,item){
            //$scope.formParams.encumbranceCostId = item[0].requestNum;
            //$scope.formParams.personName = item[0].personName;
            //$scope.formParams.departmentName = item[0].departmentName;
            //$scope.formParams.costUserName = item[0].costUserName;
            //$scope.formParams.custAccountName = item[0].custAccountName;
            //$scope.formParams.expenseAccountName = item[0].expenseAccountName;
            //$scope.formParams.expensePeriod = item[0].expensePeriod;
            //$scope.formParams.encumbranceAmount = item[0].encumbranceAmount;
            //$scope.formParams.expenseTypeStr = item[0].expenseTypeStr;
            //$scope.formParams.channelType = item[0].channelType;
            //$scope.formParams.eventDesc = item[0].eventDesc;
            //$scope.formParams.signOpinion = item[0].signOpinion;
            //var data=item[0]

            httpServer.post(URLAPI.findOaMarketingCostMediaDetailById, {
                params: JSON.stringify({'id':item[0].requestNum})
            },function(res) {
                //debugger;
                if(res.status === 'S') {
                    var data = res.data;
                    data.reimburseLineData=[{
                        departmentName: '',
                        departmentId: '',
                        expenseItemName: '',
                        expenseItemId: '',
                    ...
                    data.encumbranceCostId=data.requestNum;
                    data.costUserId = data.costUserId;
                    data.expenseAmount=0;
                    data.costUserName = data.costUserName;
                    data.departmentId = data.departmentCode;
                    data.departmentCode = data.departmentCode;
                    data.billNo='自动生成';
                    data.requestDate='';
                    data.statusStr='';
                    data.isPostPaperReceipts='Y'
                    $scope.getBuData(res.data.itemMedia[0].deliveryPurpose)
                    $scope.setFormParams(data)
                    $scope.init()
                }else {
            ...
            $scope.storeParams = {
                ...
                personId:item[0].costUserId
            }
        }
        $scope.isSuccess=function(){
            var drivenPinNum=$scope.formParams.itemMedia[0].drivenPinNum
            var activePinNum=$scope.formParams.itemMedia[0].activePinNum
            console.log(drivenPinNum)
            console.log(activePinNum)
            if(drivenPinNum&&activePinNum){
                if(activePinNum>=drivenPinNum){
                    $scope.formParams.itemMedia[0].isSuccess='Y'
                }else{
                    $scope.formParams.itemMedia[0].isSuccess='N'
                }
            }
        }
        ...
        $scope.passEvent.cusValidator = function(){
            httpServer.post(URLAPI.updateOaMarketingCostMediaReByApproval, {
                params: JSON.stringify($scope.formParams)
            }, function (res) {
                if (res.status == 'S') {
                    $scope.approvalEvent('Y', true, '通过')
                } else {
                    SIEJS.alert(res.msg, 'error', '确定');
                    return false;
                }
