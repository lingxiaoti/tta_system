...
define(['app','angularFileUpload','webconfig'], function (app,angularFileUpload,webconfig) {
    ...
    app.controller('travelApplyFormCtrl', function ($scope, $location, $rootScope, $state, $stateParams, SIEJS,$timeout,$filter,httpServer,URLAPI) {
        $scope.params = {};
        $scope.current={};
        $scope.businessCity={};
        ...
        $scope.pageView.getBusinessFormCallback = function (res) {
            if (res.status === 'S') {
                ...
                console.log(data);
                //审批中、审批通过状态可见
                if(data.status == "ALLOW" || data.status == "APPROVAL"){
                    $scope.showFlag = true;
                }else {
                    $scope.showFlag = false;
                }
