...
define(["app", "angularFileUpload",'XLSX', "pinyin","GooFlow"], function (app, angularFileUpload,XLSX, pinyin) {
    ...
    app.controller('equPonScoringDetailCtrl', ['$scope', '$parse', '$filter', '$attrs', "SIE.JS", 'httpServer', 'URLAPI', '$stateParams', 'iframeTabAction','$http','SIEJS','$timeout','$controller', function saafTableController($scope, $parse, $filter, $attrs, JS, httpServer, URLAPI, $stateParams, iframeTabAction,$http,SIEJS,$timeout,$controller) {
        ...
        var dataListOptions = $stateParams.dataListOptions;
        var dataMessage = $stateParams.dataMessage;
        $scope.params = {};
        $scope.program = {};
        $scope.flags = {};
        ...
        $scope.program.factoryAuditFlag = 'N';
        $scope.program.panelTestFlag = 'N';
        ...
        if(dataListOptions == null || dataListOptions == undefined){
            dataListOptions = '{}';
        }
        if(dataMessage == null || dataMessage == undefined){
            dataMessage = '';
        }
        $scope.flags.dataListOptions = dataListOptions;
        $scope.flags.dataMessage = dataMessage;
        ...
        $scope.btnSave = function () {
            ...
            httpServer.post(URLAPI.saveScoringInfo, {
                ...
                if ('S' == res.status) {
                    ...
                    if($scope.program.scoringStatus != '20'){
                        JS.alert('保存成功');
                    }
                } else {
        ...
        $scope.search = function (id) {
            ...
            httpServer.post(URLAPI.findScoringInfoForFlow, {
                ...
                if (res.status == 'S') {
                    ...
                    // if(($scope.program.scoringStatus == '30' && $scope.program.sensoryTestTypes == '10') || ($scope.readonly == 'N' && $scope.program.sensoryTestTypes == '20')){
                    ...
                    //查询各个子信息
                    $scope.historicParam = {}
                    console.log($scope.program.scoringNumber);
                    $scope.historicParam.code = $scope.program.scoringNumber
                    ...
                    if($scope.program.sensoryTestTypes == '10'){
                        //第三方
                        if($scope.program.scoringStatus == '30' && $scope.program.createdBy != $scope.flags.userId && $scope.program.panelTestFlag != 'Y'){
                            $scope.panelTestReadonly = 'N';
                        }else{
                            $scope.panelTestReadonly = 'Y';
                        }
                        ...
                        //内部
                        if($scope.readonly == 'N'  && $scope.program.createdBy == $scope.flags.userId && $scope.program.panelTestFlag != 'Y'){
                            $scope.panelTestReadonly = 'N';
                        }else{
                            $scope.panelTestReadonly = 'Y';
                        }
        ...
        $scope.selectQuotationInformation = function (value) {
            ...
            $scope.current.deptName = JSON.parse(sessionStorage[appName + '_successLoginInfo']).deptName
        ...
        $scope.selectScoringStandardReturn = function () {
            ...
            $scope.program.projectNumber = $scope.currentList.projectNumber;
            ...
            $scope.program.presentCooperationSupplierNumber = $scope.currentList.presentCooperationSupplierNumber;
            ...
            //查询评分Common
            $scope.searchScoringCommon();
        }

        //查询评分Common
        $scope.searchScoringCommon = function(){
            httpServer.post(URLAPI.findScoringCommon, {
                params: JSON.stringify({
                    informationId: $scope.program.informationId
                })
            }, function (res) {
                if (res.status == 'S') {
        ...
        $scope.supplierInvitationConfirm = function(){
            ...
            //校验供应商报价单状态是否全部为【已截至】
            httpServer.post(URLAPI.checkQuotationStatus, {
                'params': JSON.stringify($scope.program)
            }, function (res) {
                if ('S' == res.status) {
                    $scope.supplierInvitationConfirmDetail();
                } else {
                    JS.alert(res.msg, 'error', '确定');
                }
            }, function (error) {
                console.error(error);
                JS.alert('校验失败!', 'error', '确定');
            })
        }

        $scope.supplierInvitationConfirmDetail = function(){
            ...
            JS.confirm('确认','确认后将不能再修改供应商列表，是否确认？','确定',function() {
                    ...
                    if($scope.supplierInvitationDataTable[i].isQuit != 'Y'){
                        ...
                        httpServer.post(URLAPI.findSupplierBaseScore, {
                            ...
                            if (res.status == 'S') {
                                ...
                                var currentPerformanceScore = 0;
                                var supplierNumber = res.data[0].supplierNumber;
                                var presentCooperationSupplierNumber = $scope.program.presentCooperationSupplierNumber;
                                if(supplierNumber == presentCooperationSupplierNumber){
                                    currentPerformanceScore = res.data[0].currentPerformanceScore;
                                }

                                var obj5 = {
                                    supplierId:res.data[0].supplierId,
                                    column: res.data[0].supplierName,
                                    defaultItem: currentPerformanceScore
                                };
                ...
                $scope.flags.dataListOptions = options.substring(0, options.length - 1) + '}';
                $scope.flags.dataMessage = message;
            })
        }
        ...
        $scope.nonPriceCalculate = function(){
            ...
            //     if(defaultItem == null || defaultItem == '' || defaultItem == undefined){
            //         JS.alert("Innovation评分填写不完整!", "error", "确定");
            //         return false;
            //     }
            // }
            //
            ...
            //         JS.alert("PKG Innovation评分填写不完整!", "error", "确定");
            //         return false;
            //     }
        ...
        //QA人员保存分数
        $scope.btnQASave = function(){
            $scope.setParams();
            httpServer.post(URLAPI.saveScoringInfo, {
                'params': JSON.stringify($scope.params)
            }, function (res) {
                if ('S' == res.status) {
                    $scope.search(res.data.scoringId);
        ...
        //报价总分计算确认
        $scope.confirmScoreCalculate = function(){
            //校验分数是否填写完整
            //Panel test 感官类型为内部，校验必填
            if($scope.program.sensoryTestTypes == '10'){
                if($scope.program.panelTestFlag != 'Y'){
                    JS.alert("Panel test评分未提交确认分数!", "error", "确定");
                    return false;
                }
                for(var i = 0; i < $scope.panelTestColumns.length; i++){
                    for(var j = 0; j < $scope.productSpecsDataTable.length; j++){
        ...
        $scope.btnMoniterQuotation = function () {
            iframeTabAction('监控报价', 'monitorPriceInfo/'+$scope.program.informationId + '/Y')
        }

        $scope.showScoringStander = function(){
            var scoringStandardId = $scope.program.standardsId;
        ...
        $scope.btnSubmit = function(){
            ...
            //Factory audit
            if($scope.program.factoryAuditFlag != 'Y'){
                JS.alert("Factory audit评分未提交确认分数!", "error", "确定");
                return false;
            }
            for(var i = 0; i < $scope.factoryAuditColumns.length; i++){
                var defaultItem = $scope.factoryAuditColumns[i].defaultItem;
            ...
            httpServer.post(URLAPI.bpmStart, {
                ...
                if (res.status == 'S') {
                    $scope.search($scope.program.scoringId);
                    $scope.program.scoringStatus = '20';
                    $scope.btnSave();
                    JS.alert('提交成功');
                }
                else {
