...
define(['app', 'pinyin', 'ztree', 'XLSX', 'angularFileUpload'], function (app, pinyin, ztree, angularFileUpload, XLSX) {
    ...
    app.controller('editCsrUpdateCtrl', function (wechatFind, $filter, Base64, deleteUserResp, findProfileSqlDatas, $window,
            ...
            if (updateHeadId) {
                ...
                httpServer.post(URLAPI.findCsrUpdateHeadInfo, {
            ...
            $scope.uploadFileForHead = function () {
                ...
                $http.post(URLAPI.fileUploadForCsrUpdateHead, fd, {
            ...
            $scope.validate = function () {
                for (var i = 0; i < $scope.dataTable.data.length; i++) {
                    ...
                    if ($scope.dataTable.data[i].score == undefined) {
            ...
            $scope.btnSave = function (status) {
                ...
                httpServer.post(URLAPI.saveCsrUpdateHeadAndLine, {
                    'editParams': JSON.stringify($scope.editParams)
                }, function (res) {
                    if (res.status == "S") {
            ...
            $scope.fileUploadForLine = function (element) {
                ...
                fd.append('file', file);
                fd.append("fileName", fileName);
                $http.post(URLAPI.fileUploadForCsrUpdateLine, fd, {
                    transformRequest: angular.identity,
                    headers: {
                        'Certificate': sessionStorage[appName + '_certificate'] || localStorage[appName + '_certificate'] || 'nothing',
                        'Content-Type': undefined
            ...
            $scope.btnDel = function (row, index) {
                ...
                SIEJS.confirm('删除', '是否确定删除？', '确定', function () {
                    if (updateLineId == null || updateLineId == '') {
                        ...
                        httpServer.post(URLAPI.deleteCsrUpdateLine, {
