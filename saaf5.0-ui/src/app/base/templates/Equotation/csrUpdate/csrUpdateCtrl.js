...
define(["app"], function (app) {
    app.controller('csrUpdateCtrl', ['$scope', '$state','URLAPI','$stateParams', 'iframeTabAction', "SIE.JS", 'httpServer', function ($scope, $state,URLAPI,$stateParams,iframeTabAction, JS, httpServer) {
        $scope.dataTable = {};
        ...
        $scope.btnNew = function () {
            iframeTabAction('年度CSR审核导入更新新增', 'editCsrUpdate/')
        };

        $scope.toEditHtml = function (row) {
            iframeTabAction('年度CSR审核导入更新', 'editCsrUpdate/'+row.updateHeadId)
        };

        $scope.deleteData = function (row,index) {
            JS.confirm('删除', '是否确定删除？', '确定', function () {
                httpServer.post(URLAPI.deleteCsrUpdateHead, {
