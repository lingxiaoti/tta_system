...
define(['app', 'pinyin', 'ztree', 'XLSX', 'angularFileUpload'], function (app, pinyin, ztree, angularFileUpload, XLSX) {
    ...
    app.controller('editQualityUpdateCtrl', function (wechatFind, $filter, Base64, deleteUserResp, findProfileSqlDatas, $window,
            ...
            $scope.uploadFileForHead = function () {
                ...
                $http.post(URLAPI.fileUploadForQualityUpdateHead, fd, {
            ...
            $scope.validate = function () {
                for (var i = 0; i < $scope.dataTable.data.length; i++) {
                    ...
                    if ($scope.dataTable.data[i].category == null || $scope.dataTable.data[i].category == undefined) {
                        SIEJS.alert("品类不能为空!", 'error', '确定');
                        return true;
                    }
                    ...
                    if ($scope.dataTable.data[i].result == null || $scope.dataTable.data[i].result == undefined) {
                        SIEJS.alert("结果不能为空!", 'error', '确定');
                        return true;
                    }
            ...
            $scope.btnSave = function (status) {
                ...
                httpServer.post(URLAPI.saveQualityUpdateHeadAndLine, {
                    'editParams': JSON.stringify($scope.editParams)
                }, function (res) {
                    if (res.status == "S") {
                        // 保存后刷新
            ...
            $scope.fileUploadForLine = function (element) {
                ...
                fd.append('file', file);
                fd.append("fileName", fileName);
                $http.post(URLAPI.fileUploadForQualityUpdateLine, fd, {
                    transformRequest: angular.identity,
                    headers: {
                        'Certificate': sessionStorage[appName + '_certificate'] || localStorage[appName + '_certificate'] || 'nothing',
                        'Content-Type': undefined
