...
define(['app', 'pinyin', 'ztree', 'angularFileUpload',"GooFlow"], function (app, pinyin, ztree, angularFileUpload) {
    ...
    app.controller('equPosSuspendEditCtrl', function (wechatFind, $filter, Base64, deleteUserResp, findProfileSqlDatas, $window,
        ...
        // ------------------------------------------------初始化------------------------------------------------
        // var supSuspendId = $stateParams.supSuspendId
        if ($stateParams.supSuspendId){
            var supSuspendId = $stateParams.supSuspendId;
        }else if ($scope.urlParams.businessKey){
            var supSuspendId = $scope.urlParams.businessKey;
        }
        $scope.params = {}
        $scope.param = {}
...
// ------------------------------------------------调整------------------------------------------------
        $scope.changeShowFlag = function () {
            ...
            if($scope.params.supSuspendStatus!='20'&&$scope.params.supSuspendStatus!='30'){
                $scope.saveFlag = true;  //提交与审批状态不能保存
                $scope.submitFlag = true;
            }
            if($scope.params.supSuspendStatus=='20'){
                $scope.approveFlag = true;  //提交与审批状态不能保存
                $scope.rejectFlag= true;
            }
            if($scope.params.supSuspendStatus=='50'&&saafsuccessLoginInfo.userId == $scope.params.createdBy){
                $scope.cancelFlag = true;   //驳回状态本人可以取消
            }
            if($scope.params.supSuspendStatus=='10'||$scope.params.supSuspendStatus=='50'){
                $scope.editFlag= true; //拟定与驳回状态才能编辑
            }
        }

        $scope.changeType = function () {
            if($scope.params.supSuspendType==='10'){
                $scope.params.dateName = '暂停时间';
                $scope.params.reasonName = '暂停原因';
            }else if ($scope.params.supSuspendType==='20'){
                $scope.params.dateName = '淘汰时间';
                $scope.params.reasonName = '淘汰原因';
            }
        }

        if (supSuspendId) {
            $scope.params.supSuspendId = supSuspendId;
            httpServer.post(URLAPI.findSupSuspendDatail, {
                // params: JSON.stringify($scope.params)
                params: JSON.stringify({
                    supSuspendId: supSuspendId
                })
            }, function (res) {
                if (res.status == "S") {
                    ...
                    $scope.historicParam = {}
                    $scope.historicParam.code = $scope.params.supSuspendCode
            ...
            $scope.params.supSuspendType = '10';
            $scope.params.supSuspendStatus = '10'
...
// ------------------------------------------------保存------------------------------------------------
        ...
        $scope.save = function (action) {
            if ($scope.params.supplierName == null || $scope.params.supplierName == ""
            ...
            } else if  ($scope.params.supSuspendReason == null || $scope.params.supSuspendReason == ""
            ) {
                SIEJS.alert("请填写原因。", "error", "确定");
            }else {
                ...
                httpServer.post(URLAPI.saveEquPosSuspend, {
                    'params': JSON.stringify($scope.params),
                }, function (res) {
                    if (res.status == "S") {
                        $scope.params.supSuspendCode = res.data.supSuspendCode;
                        $scope.params.supSuspendStatus = res.data.supSuspendStatus;
                        $scope.params.creationDate = res.data.creationDate;
                        $scope.params.supSuspendId = res.data.supSuspendId;
                        $scope.changeShowFlag();
        ...
        //获取单据ID
        function getId() {
            return $scope.urlParams.businessKey ? $scope.urlParams.businessKey : $scope.params.supSuspendId;
        }
        ...
        $scope.processImageParams = {
            ...
            key: 'EQU_ZTTTSH.-999' //流程唯一标识，在流程管理->流程设计->设计 流程中获取，流程配置时修改为对应表单的流程唯一标识
        };
        // tab 切换
        ...
        $scope.getProcessInfo = function (callback) {
            httpServer.post(URLAPI.processGet, {
                    'params': JSON.stringify({
                        ...
                        businessKey:$scope.params.supSuspendId
                    })
                },
                function (res) {
                    if (res.status === 'S') {
        ...
        $scope.btnSubmit = function(){
            if($scope.params.supSuspendStatus != '10' && $scope.params.supSuspendStatus != '40'){
            ...
            $scope.paramsBpm={
                ...
                "respId": "990021",
                "processDefinitionKey": "EQU_ZTTTSH.-999", //流程唯一标识，需修改为对应业务表单流程唯一标识
                "saveonly": false,
                "businessKey": $scope.params.supSuspendId, //单据ID
                "title": "供应商暂停/淘汰审批流程", //流程标题
                "positionId": 0,
                "orgId": 0,
                "userType": "30",
                "billNo": $scope.params.supSuspendCode
            }
            httpServer.post(URLAPI.bpmStart, {
                'params': JSON.stringify($scope.paramsBpm)
            }, function (res) {
                if (res.status == 'S') {
                    $scope.params.supSuspendStatus = '20'
                    $scope.changeShowFlag();
                    SIEJS.alert('提交成功');
                    // $scope.search($scope.params.supSuspendId);
                    // httpServer.post(URLAPI.saveEquPosSuspendSubmit, {
                    //     'params': JSON.stringify($scope.params)
                    // }, function (res) {
                    //     if (res.status == 'S') {
                    //         // $scope.search($scope.params.supSuspendId);
