...
define(['app', 'pinyin', 'ztree', 'XLSX', 'angularFileUpload'], function (app, pinyin, ztree, angularFileUpload, XLSX) {
    ...
    app.controller('completeQuotationInfoCtrl', function (wechatFind, $filter, Base64, deleteUserResp, findProfileSqlDatas, $window,
        ...
        $scope.findQuotationInfo = function () {
            ...
            httpServer.post(URLAPI.findQuotationInfo, {
                ...
                if (res.status == 'S') {
                    ...
                    console.log($scope.editParams.orderType);
                }
            }, function (error) {
                console.error(error)
            })
        ...
        $scope.validate = function () {
            ...
            for (var i = 0; i < $scope.productDataTable.length; i++) {
                if ($scope.productDataTable[i].firstProductPrice == '' || $scope.productDataTable[i].firstProductPrice == undefined) {
                    SIEJS.alert('产品报价表承载产品的包装价格,但不含合同费用及不含税Net Net Cost(元)不能为空', 'error', '确定');
                    return true;
                }
                if ($scope.productDataTable[i].twoProductPrice == '' || $scope.productDataTable[i].twoProductPrice == undefined) {
            ...
            // 校验页面下方百分数必填
            if ($scope.editParams.firstPercent == '' || $scope.editParams.firstPercent == undefined) {
                SIEJS.alert('产品备注不能为空', 'error', '确定');
                return true;
            }
            if ($scope.editParams.secondPercent == '' || $scope.editParams.secondPercent == undefined) {
                SIEJS.alert('产品备注不能为空', 'error', '确定');
                return true;
            }
        };
