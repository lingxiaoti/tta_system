...
define(['app', 'pinyin', 'ztree', 'XLSX', 'angularFileUpload'], function (app, pinyin, ztree, angularFileUpload, XLSX) {
    ...
    app.controller('editContractUpdateCtrl', function (wechatFind, $filter, Base64, deleteUserResp, findProfileSqlDatas, $window,
        ...
        if (updateHeadId) {
            ...
            httpServer.post(URLAPI.findContractUpdateHeadInfo, {
                ...
                if (res.status == "S") {
                    ...
                    $scope.headData.deptName = JSON.parse(sessionStorage[appName + '_successLoginInfo']).departmentName;
        ...
        $scope.uploadFileForHead = function () {
            ...
            $http.post(URLAPI.fileUploadForContractUpdateHead, fd, {
        ...
        $scope.btnSave = function (status) {
            ...
            httpServer.post(URLAPI.saveContractUpdateHeadAndLine, {
                'editParams': JSON.stringify($scope.editParams)
            }, function (res) {
                if (res.status == "S") {
                    // 保存后刷新
        ...
        $scope.fileUploadForLine = function (element) {
            ...
            fd.append('file', file);
            fd.append("fileName", fileName);
            $http.post(URLAPI.fileUploadForContractUpdateLine, fd, {
                transformRequest: angular.identity,
                headers: {
                    'Certificate': sessionStorage[appName + '_certificate'] || localStorage[appName + '_certificate'] || 'nothing',
                    'Content-Type': undefined
        ...
        $scope.btnDel = function (row, index) {
            ...
            SIEJS.confirm('删除', '是否确定删除？', '确定', function () {
                if (updateLineId == null || updateLineId == '') {
                    ...
                    httpServer.post(URLAPI.deleteContractUpdateLine, {
