...
define(['app', 'pinyin', 'ztree','XLSX','fixedReport'], function (app, pinyin, ztree,XLSX,fixedReport) {
    app.controller('ttaContractRecordHeaderCtrl', function ($scope, $filter, $location, $timeout,$rootScope, $state, $stateParams, SIEJS, httpServer,validateForm, URLAPI, iframeTabAction, setNewRow,$http,lookupCode,tableXlsExport,saafLoading) {
        ...
        $scope.click = function (index) {
            $timeout(function(){
                document.getElementById("id_remake" + index).focus();
            },0)
        ...
        $scope.getNameL = function () {
            $('#nameTtaContractRecordHeaderLov').modal('toggle')
        };
        ...
        $scope.selectSupplierReturn = function (key, value, currentList) {
            var row = $scope.ttaContractRecordHeaderTable.data[$scope.ttaContractRecordHeaderTable.data.selectRow];
            for (var i = 0; i < $scope.ttaContractRecordHeaderTable.data.length; i++) {
                var recordHeaderRow = $scope.ttaContractRecordHeaderTable.data[i];
                var year = recordHeaderRow.year;
                var vendorNbr = recordHeaderRow.vendorNbr;
                var receiveStatus = (recordHeaderRow.receiveStatus === undefined || recordHeaderRow.receiveStatus == null
                    || recordHeaderRow.receiveStatus === "") ? "A" : recordHeaderRow.receiveStatus;
                if (year ==currentList[0].proposalYear && vendorNbr === currentList[0].vendorCode && receiveStatus !== "C") {//领用状态不等于作废的
                    SIEJS.alert('系统中已存在同一年度【'+ year + '】,同一个供应商【' + vendorNbr + '】的业务合同书编号【' + recordHeaderRow.logNo+ '】', "error", "确定");
                    return;
                }
            }
            httpServer.post(URLAPI.findContractVendorTtaContractRecordHeader,
                {params: JSON.stringify({
                        year:currentList[0].proposalYear,
                        vendorNbr:currentList[0].vendorCode
                    })},
                function (res) {
                    if (res.status == "S") {
