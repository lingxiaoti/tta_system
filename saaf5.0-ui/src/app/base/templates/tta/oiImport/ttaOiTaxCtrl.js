...
define(['app', 'pinyin', 'ztree','XLSX'], function (app, pinyin, ztree,XLSX) {
    app.controller('ttaOiTaxCtrl', function ($scope, $filter, $location, $rootScope, $state, $stateParams, SIEJS, httpServer, URLAPI, iframeTabAction, setNewRow,$http, saafLoading) {
        $scope.buttonFlag = true ;
        $scope.btnDel = function () {
            var row = $scope.dataTable.selectRow;
            SIEJS.confirm('您确认要删除吗？', '', '确定', function () {
                httpServer.post(
                    URLAPI.deleteImportTaxInfo,

                    {
                        'params': JSON.stringify({
                            "ttaOiTaxId": row.ttaOiTaxId,
                            flag :'single'
                        })
                    },
                    function (res) {
        ...
        $scope.save = function (invalid) {
            ...
            $http.post(URLAPI.saveImportTaxInfo, fd, {
