...
define(['app', 'pinyin', 'ztree','XLSX'], function (app, pinyin, ztree,XLSX) {
    app.controller('ttaOiSummaryLineCtrl', function ($scope, $filter, $location, $rootScope, $state, $stateParams, SIEJS, httpServer, URLAPI, iframeTabAction, setNewRow,$http, saafLoading) {
        $scope.buttonFlag = true ;
        $scope.btnDel = function () {
            var row = $scope.dataTable.selectRow;
            SIEJS.confirm('您确认要删除吗？', '', '确定', function () {
                httpServer.post(
                    URLAPI.deleteImportOISInfo,

                    {
                        'params': JSON.stringify({
                            "ttaOiSummaryLineId": row.ttaOiSummaryLineId,
                            flag :'single'
                        })
                    },
                    function (res) {
        ...
        $scope.save = function (invalid) {
            ...
            $http.post(URLAPI.saveImportOISInfo, fd, {
