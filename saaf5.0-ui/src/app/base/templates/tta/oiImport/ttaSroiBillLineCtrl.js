'use strict';
define(['app', 'pinyin', 'ztree','jqueryUi','XLSX'], function (app, pinyin, ztree,jqueryUi,XLSX) {
    app.controller('ttaSroiBillLineCtrl', function ($scope, $filter, $location, $rootScope, $state, $stateParams, SIEJS, httpServer, URLAPI, iframeTabAction, setNewRow,$http, saafLoading) {
        $scope.buttonFlag = true ;
        $scope.messageCurrentStage =   ''  ;
        $scope.messageCurrentOrderNum = '' ;
        $scope.btnDel = function () {
            var row = $scope.dataTable.selectRow;
            SIEJS.confirm('您确认要删除吗？', '', '确定', function () {
                httpServer.post(
                    URLAPI.deleteImportSROIInfo,

                    {
                        'params': JSON.stringify({
                            "ttaSroiBillImportId": row.ttaSroiBillImportId,
                            flag :'single'
                        })
                    },
                    function (res) {
        ...
        $scope.PLSC = function () {
            $scope.paramsPL = {} ;
            $('#PLDeleteTtaSroiBillLine').modal('toggle')
        }
        // 查询导出结果
        ...
        $scope.daoru = function () {
            $('#excelTtaSroiBillLine').modal('toggle');
            $scope.buttonFlag = true ;
            $scope.messageCurrentStage = '';
            $scope.messageCurrentOrderNum =  '';
        }
        ...
        $scope.save = function (invalid) {
            ...
            $http.post(URLAPI.saveImportSROIInfo, fd, {
