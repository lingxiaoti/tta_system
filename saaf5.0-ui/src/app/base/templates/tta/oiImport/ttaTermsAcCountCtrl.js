...
define(['app', 'pinyin', 'ztree','XLSX'], function (app, pinyin, ztree,XLSX) {
    app.controller('ttaTermsAcCountCtrl', function ($scope, $filter, $location, $rootScope, $state, $stateParams, SIEJS, httpServer, URLAPI, iframeTabAction, setNewRow,$http,saafLoading) {
        $scope.buttonFlag = true ;
        $scope.btnDel = function () {
            var row = $scope.dataTable.selectRow;
            SIEJS.confirm('您确认要删除吗？', '', '确定', function () {
                httpServer.post(
                    URLAPI.deleteImportACInfo,

                    {
                        'params': JSON.stringify({
                            "ttaTermsAcCountId": row.ttaTermsAcCountId,
                            flag :'single'
                        })
                    },
                    function (res) {
        ...
        $scope.PLSC = function () {
            $scope.paramsPL = {} ;
            $('#PLDeleteTtaTermsAcCount').modal('toggle')
        }
        // 查询导出结果
        ...
        $scope.daoru = function () {
            $('#excelTtaTermsAcCount').modal('toggle');
            $scope.buttonFlag = true ;
        }
        ...
        $scope.save = function (invalid) {
            ...
            $http.post(URLAPI.saveImportACInfo, fd, {
