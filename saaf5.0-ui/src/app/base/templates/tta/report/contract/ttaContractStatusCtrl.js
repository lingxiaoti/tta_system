...
define(['app', 'pinyin', 'ztree'], function (app, pinyin, ztree) {
    ...
    app.controller('ttaContractStatusCtrl', function ($scope, $filter, $location, $rootScope, $state, $stateParams, SIEJS, httpServer, URLAPI, iframeTabAction, setNewRow,lookupCode,$timeout) {
        $scope.searchParam = {};
        var date=new Date;
        ...
        $scope.searchDataTable = function () {
            ...
            httpServer.post(URLAPI.findContractNodeList,
...
/*                                if (j == 22) {
                                ...
                                if (j == (22 + 3)) {
                                    //累加并填充到In the pending approved列
                                    rowDataList[19 + 3].nodeValue = inThePendingApproved;
                                    //计算第4列(In the preparation)  3=1-2-19-20
                                    rowDataList[3].nodeValue = parseInt(rowDataList[1].nodeValue) - parseInt(rowDataList[2].nodeValue) - parseInt(rowDataList[19 + 3].nodeValue) - parseInt(rowDataList[20 + 3].nodeValue);
                                    //Rate% for proposal completion 角标22 = Proposal approved 角标20/Proposal need buyer sumbit to BIC 角标1
                                    //计算列的值Rate% for proposal completiony
                                    rowDataList[22 + 3].nodeValue = parseFloat(rowDataList[1].nodeValue) == 0 ? "Infinity" : ((parseFloat(rowDataList[20 + 3].nodeValue)* 100 /parseFloat(rowDataList[1].nodeValue)).toFixed(2) + '%');
                                    //In the pending for Buyer Submition 角标18 = pending to submit 角标2 + In the preparation 角标3
                                    rowDataList[18 + 3].nodeValue = parseFloat(rowDataList[2].nodeValue) + parseFloat(rowDataList[3].nodeValue);
                                    // Rate% for In the pending for Buyer Submition-角标21=In the pending for Buyer Submition-角标18/Proposal need buyer sumbit to BIC-角标1
                                    rowDataList[21 + 3].nodeValue = parseFloat(rowDataList[1].nodeValue) == 0 ? "Infinity" : ((parseFloat(rowDataList[18 + 3].nodeValue) * 100 /parseFloat(rowDataList[1].nodeValue)).toFixed(2) + '%');
                                }
                            }
                        }
                        ...
                        lastRowData[21 + 3].nodeValue = parseInt(lastRowData[1].nodeValue) == 0 ? "Infinity" : ((parseInt(lastRowData[18 + 3].nodeValue) * 100 / parseInt(lastRowData[1].nodeValue)).toFixed(2) + '%');
                        //Rate% for proposal completion_22角标 = Proposal  approved_角标20/Proposal need buyer sumbit to BIC_角标1
                        lastRowData[22 + 3].nodeValue =  parseInt(lastRowData[1].nodeValue) == 0 ? "Infinity" : ((parseInt(lastRowData[20 + 3].nodeValue) * 100 / parseInt(lastRowData[1].nodeValue)).toFixed(2) + '%');
                    } else {
                        SIEJS.alert(res.msg, "error", "确定");
                    }
                }, function (err) {
