...
define(['app', 'pinyin', 'ztree','XLSX','fixedReport'], function (app, pinyin, ztree,XLSX, fixedReport) {
    app.controller('ttaProcessNPPcheckingReportCtrl', function ($scope, $filter, $location, $rootScope, $state, $stateParams, SIEJS, httpServer,validateForm, URLAPI, iframeTabAction, setNewRow,$http,$timeout,lookupCode,saafLoading,
        ...
        $scope.$watch('nPPTable.data', function (newVal, oldVal) {
        ...
        $scope.submitApproval = function (form) {
            if($scope.nPPTable.length == 0){
                SIEJS.alert('当前页面没有数据,不允许提交审批', 'error', '确定');
                return;
            }else if($scope.nPPTable[0].headerStatus != 'DS01'){
            ...
            $scope.paramsBpm = {
                ...
                "saveonly": false,
                "businessKey": $scope.nPPTable[0].id, //单据ID
                "title": "DM_REPORT_审批" + $scope.nPPTable[0].batchCode, //流程标题，修改为当前业务信息
                "positionId": 0,
                "orgId": 0,
                "userType": "20",
                "billNo": $scope.nPPTable[0].batchCode
            };
        ...
        $scope.btnSave = function (form){
            ...
            $scope.saveAll =  $scope.nPPTable.data ;
        ...
        $scope.windowSplit = function (){
            ...
            if (!$scope.nPPTable.data || !$scope.nPPTable.data.selectRowData) {
                SIEJS.alert("请选择一行数据进行修改!", "error", "确定");
                return;
            }
            $scope.currentRow = $scope.nPPTable.data.selectRowData;
            $scope.splitDataTable = [];
               /* [
        ...
        $scope.purchaseFun = function (index,currentValue,lookUpData){
            var row = $scope.nPPTable.data[index] ;
        ...
        $scope.selectTransferReturn = function (key, value, currentList){
            ...
            if (!$scope.nPPTable.data.selectRowData){
                SIEJS.alert("请选择一行", "error", "确定");
                return ;
            }
            var pLins = [];
            pLins.push($scope.nPPTable.data.selectRowData) ;
        ...
        $scope.afterGetData =function () {
            for(var i = 0; i < $scope.nPPTable.data.length; i ++) {
                //获取采购行动
                var currentValue = $scope.nPPTable.data[i].purchase ;
                ...
                if($scope.lookupCodeParty){
                    //获取 豁免原因_1
                    $scope.nPPTable.data[i].exemptionReasonList = $scope.lookupCodeParty.filter(function (x) {
                        return x.lookupType == 'TTA_DM_REASON' && x.parentLookupValuesId == lookUpData[0].lookupValuesId;
                    });
                }

                //获取 豁免原因_2
                var currentValue2 = $scope.nPPTable.data[i].exemptionReason ;
                var values2  = $scope.lookupCodeParty.filter(function (x) {
                    return x.lookupType == 'TTA_DM_REASON' &&  x.lookupCode  == currentValue2;
                });
                if($scope.lookupCodeParty){
                    $scope.nPPTable.data[i].exemptionReasonList2 = $scope.lookupCodeParty.filter(function (x) {
