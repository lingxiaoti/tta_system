...
define(['app', 'pinyin', 'ztree','XLSX','fixedReport'], function (app, pinyin, ztree,XLSX, fixedReport) {
    app.controller('ttaProcessHWBCheckingReportCtrl', function ($scope, $filter, $location, $rootScope, $state, $stateParams, SIEJS, httpServer,validateForm, URLAPI, iframeTabAction, setNewRow,$http,$timeout,lookupCode,saafLoading,
        ...
        $scope.$watch('hwbTable.data', function (newVal, oldVal) {
        ...
        $scope.submitApproval = function (form) {
            if($scope.hwbTable.length == 0){
                SIEJS.alert('当前页面没有数据,不允许提交审批', 'error', '确定');
                return;
            }else if($scope.hwbTable[0].headerStatus != 'DS01'){
            ...
            $scope.paramsBpm = {
                ...
                "saveonly": false,
                "businessKey": $scope.hwbTable[0].id, //单据ID
                "title": "DM_REPORT_审批" + $scope.hwbTable[0].batchCode, //流程标题，修改为当前业务信息
                "positionId": 0,
                "orgId": 0,
                "userType": "20",
                "billNo": $scope.hwbTable[0].batchCode
            };
        ...
        $scope.btnSave = function (form){
            ...
            $scope.saveAll =  $scope.hwbTable.data ;
        ...
        $scope.windowSplit = function (){
            $("#hwbTableId").freeze({ height:500,width:1416, rows: 2, cols: 8 });

            if (!$scope.hwbTable.data || !$scope.hwbTable.data.selectRowData) {
                SIEJS.alert("请选择一行数据进行修改!", "error", "确定");
                return;
            }
            $scope.currentRow = $scope.hwbTable.data.selectRowData;
            $scope.splitDataTable = [];
            /* [
        ...
        $scope.purchaseFun = function (index,currentValue,lookUpData){
            var row = $scope.hwbTable.data[index] ;
        ...
        $scope.selectTransferReturn = function (key, value, currentList){
            ...
            if (!$scope.hwbTable.data.selectRowData){
                SIEJS.alert("请选择一行", "error", "确定");
                return ;
            }
            var pLins = [];
            pLins.push($scope.hwbTable.data.selectRowData) ;
        ...
        $scope.afterGetData =function () {
            for(var i = 0; i < $scope.hwbTable.data.length; i ++) {
                //获取采购行动
                var currentValue = $scope.hwbTable.data[i].purchase ;
                ...
                if($scope.lookupCodeParty){
                    //获取 豁免原因_1
                    $scope.hwbTable.data[i].exemptionReasonList = $scope.lookupCodeParty.filter(function (x) {
                        return x.lookupType == 'TTA_DM_REASON' && x.parentLookupValuesId == lookUpData[0].lookupValuesId;
                    });
                }

                //获取 豁免原因_2
                var currentValue2 = $scope.hwbTable.data[i].exemptionReason ;
                var values2  = $scope.lookupCodeParty.filter(function (x) {
                    return x.lookupType == 'TTA_DM_REASON' &&  x.lookupCode  == currentValue2;
                });
                if($scope.lookupCodeParty){
                    $scope.hwbTable.data[i].exemptionReasonList2 = $scope.lookupCodeParty.filter(function (x) {
