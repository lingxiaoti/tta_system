...
define(['app', 'pinyin', 'ztree','XLSX'], function (app, pinyin, ztree,XLSX) {
    app.controller('ttaHwbBaseLineCtrl', function ($scope, $filter, $location, $rootScope, $state, $stateParams, SIEJS, httpServer, URLAPI, iframeTabAction, setNewRow,$http) {
        $scope.buttonFlag = true ;
        $scope.btnDel = function () {
            var row = $scope.dataTable.selectRow;
            SIEJS.confirm('您确认要删除吗？', '', '确定', function () {
                httpServer.post(
                    URLAPI.deleteImportHwbBaseInfo,

                    {
                        'params': JSON.stringify({
                            "hwbBaseLineId": row.hwbBaseLineId,
                            flag :'single'
                        })
                    },
                    function (res) {
        ...
        $scope.daoru = function () {
            $('#excelTtaHwbBaseLine').modal('toggle');
            $scope.buttonFlag = true ;
        }
        ...
        $scope.save = function (invalid) {
            ...
            $http.post(URLAPI.saveImportHwbBaseInfo, fd, {
