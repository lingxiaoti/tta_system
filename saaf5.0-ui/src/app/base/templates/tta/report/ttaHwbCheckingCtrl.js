...
define(['app', 'pinyin', 'ztree','XLSX','fixedReport'], function (app, pinyin, ztree,XLSX,fixedReport) {
    app.controller('ttaHwbCheckingCtrl', function ($scope, $filter, $location, $timeout,$rootScope, $state, $stateParams, SIEJS, httpServer,validateForm, URLAPI, iframeTabAction, setNewRow,$http,lookupCode,tableXlsExport,saafLoading) {
        ...
        $scope.aboiParams = {};
        $scope.ttaHwbfileDataTable= {};
        $scope.fileDataParams = {};
        $scope.currentList2 = [] ;
        $scope.batchId = $stateParams.batchId;
        $scope.params = {"batchCode":$scope.batchId,"orderBy":'receiveAmount',"hwbType":""} ;
        ...
        $scope.saveNppPurchaseReply = function (form) {
            ...
            for(var i = 0; i < $scope.currentList2.length; i ++) {
                ...
                var lookUpData = $scope.lookupCodeParty.filter(function (x) {
                    return currentValue && x.lookupType == 'TTA_HWB_ACTION' && x.lookupCode == currentValue;
                });
                //获取豁免原因1
                if($scope.lookupCodeParty && lookUpData.length >0 ){
                    ...
                    $scope.cWTable.data[i].exemptionReasonList = $scope.lookupCodeParty.filter(function (x) {
                        return x.lookupType == 'TTA_HWB_REASON' && x.parentLookupValuesId == lookUpData[0].lookupValuesId;
                    });
                }
                ...
                var values2  = $scope.lookupCodeParty.filter(function (x) {
                    return currentValue2 && x.lookupType == 'TTA_HWB_REASON' &&  x.lookupCode  == currentValue2;
                });
                if($scope.lookupCodeParty && values2.length >0){
                    $scope.cWTable.data[i].exemptionReasonList2 = $scope.lookupCodeParty.filter(function (x) {
                        return x.lookupType == 'TTA_HWB_REASON1' && x.parentLookupValuesId == values2[0].lookupValuesId;
                    });
                }
            }
            $('#nppPurchaseReplyModal').modal('toggle');
...
//选择采购行动
        $scope.purchaseReplyFun = function (key) {
            ...
            var values = $scope.lookupCodeParty.filter(function (x) {
                return  x.lookupCode  == key && x.lookupType == 'TTA_HWB_ACTION';
            });
            if($scope.lookupCodeParty){
                //获取 豁免原因_1
                $scope.currentExemptionReasonList = $scope.lookupCodeParty.filter(function (x) {
                    return x.lookupType == 'TTA_HWB_REASON' && x.parentLookupValuesId == values[0].lookupValuesId;
                });
            }
        };
        ...
        $scope.exemptionReasonListChangeByModal = function () {
            ...
            if ($scope.lookupCodeParty) {
                ...
                $scope.currentExemptionReasonList2 = $scope.lookupCodeParty.filter(function (x) {
                    return x.lookupType == 'TTA_HWB_REASON1' && x.parentLookupValuesId == values[0].lookupValuesId;
                });
            }
        };
        ...
        $scope.btnSave = function (data,type){
            // form 这个保存不校验 改成生成的时候校验
            // if(!validateForm(form)){
            //     return;
            // }
            $scope.saveAll =  data ;
            httpServer.post(URLAPI.saveOrUpdateTtaHwbMajorAll,
                {params: JSON.stringify({
                        save:$scope.saveAll
                    })},
                function (res) {
        ...
        $scope.getNameL =  function (){
            $('#lovHwbReportAdjustByFind').modal('toggle') ;
        };
        ...
        $scope.selectTransferReturn = function (key, value, currentList){
            ...
            httpServer.post(URLAPI.saveOrUpdateTtaHwbMajorTransferALL,
                {params: JSON.stringify({
                        lines:pLins,
        ...
        $scope.saveWindow = function (){
            httpServer.post(URLAPI.saveOrUpdateTtaHwbMajorSplitALL,
                {params: JSON.stringify({
        ...
        $scope.fileInfoFunc = function (row) {
            $scope.fileDataParams.functionId = 'tta_hwb_checking_report';
            $scope.fileDataParams.businessId = row.hwbId;
            $scope.ttaHwbfileDataTable.search(1);
        ...
        $scope.uploadFile = function (form,params) {
            ...
            var sideAgrtHId = $scope.cWUploadRow.hwbId;
            ...
            fd.append("functionId", "tta_hwb_checking_report");
        ...
        $scope.delUploadFile = function () {
            var item = $scope.ttaHwbfileDataTable.selectRow;
        ...
        $scope.check = function (item, e, t) {
            ...
            var currentIndex = -1;
            for (var i = 0; i < $scope.currentList2.length; i++) {
                if (item.hwbId === $scope.currentList2[i].hwbId) {
                    currentIsPush = false;
                    currentIndex = i;
                    break;
                }
        ...
        $scope.btnCreate = function (form){
            ...
            for (var i = 0; i < $scope.currentList2.length; i++) {
                ...
                //采购行动为“收取”或者“折扣收取”的，采购确认金额需大于0
                if (row.purchase && (row.purchase === 'A01' || row.purchase === 'A10') && row.noUnconfirmAmount <= 0) {
                    SIEJS.alert("您选择的供应商编码为" + row.parentVendorCode + "那一行数据的采购行动为“收取”或者“折扣收取”，采购确认金额需大于0", "error", "确定");
                    return;
                }
            }
             if(!validateForm(form)){
        ...
        $scope.btnCreateEnd = function () {
            httpServer.post(
                ...
                {
                    'params': JSON.stringify({"lines": $scope.currentList2,"reprotTypeName":'HWB'})
                },
                function (res) {
                    if (res.status == 'S') {
            ...
            if ($scope.cWTable.data) {
                ...
                if($scope.lookupCodeParty){
                    var totalParams =  $scope.lookupCodeParty.filter(function (x) {
                        return  x.lookupType == 'TTA_HWB_ACTION'  || x.lookupType =='TTA_HWB_REASON' || x.lookupType =='TTA_HWB_REASON1';
                    });

                    var actionParams = totalParams.filter(function (x) {
                        return x.lookupType == 'TTA_HWB_ACTION' ;
                    });

                    var reasonParams = totalParams.filter(function (x) {
                        return x.lookupType == 'TTA_HWB_REASON' ;
                    });

                    var reasonOneParams = totalParams.filter(function (x) {
                        return x.lookupType == 'TTA_HWB_REASON1' ;
                    });
                }
        ...
        $scope.btnExportTemplate = function (){
            $('#findReportExportCwId').modal('toggle');
        };
        /**
         * 导出批次确认
         */
        $scope.selectReportExportCwReturn = function (key, value, currentList){
        ...
        $scope.uploadSave = function (invalid) {
            ...
            $http.post(URLAPI.updateImportHwbInfo, fd, {
                transformRequest: angular.identity,
                headers: {
                    'Certificate': sessionStorage[appName + '_certificate'] || localStorage[appName + '_certificate'] || 'nothing',
                    'Content-Type': undefined
        ...
        $scope.changeHwbBrand = function (row) {
            var brandCn = row.brandCn;
            var content = row.content;
            var split = content.split("-");
            split[3] = brandCn;
            row.content = split.join("-");
        };
