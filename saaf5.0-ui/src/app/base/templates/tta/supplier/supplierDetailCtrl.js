...
define(['app', 'pinyin', 'ztree', 'angularFileUpload'], function (app, pinyin, ztree, angularFileUpload) {
    ...
    app.controller('supplierDetailCtrl', function ($scope, $filter, $location, $rootScope, $state, $stateParams, SIEJS, httpServer, URLAPI, iframeTabAction, $http) {
        $scope.userData = JSON.parse(localStorage.getItem(appName + '_successLoginInfo'));
        var id = $stateParams.id;
       // $scope.isShow = $scope.userData.userType == '45' ? true : false;
        ...
        $scope.fileInfo = {};
        $scope.relSupplierTable = {};
        $scope.supplierDeptTable = {};
        $scope.supplierBrandTable = {};
