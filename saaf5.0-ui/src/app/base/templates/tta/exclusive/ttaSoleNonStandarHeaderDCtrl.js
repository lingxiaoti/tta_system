...
define(['app','ztree'],function(app,ztree) {
    app.controller('ttaSoleNonStandarHeaderDCtrl',['$scope', 'httpServer', 'URLAPI', 'SIE.JS','$stateParams','validateForm','iframeTabAction','$sce','$timeout','poorYear',
        function($scope,httpServer,URLAPI,SIEJS,$stateParams,validateForm,iframeTabAction,$sce,$timeout,poorYear) {
            ...
            $scope.processImageParams = {
                ...
                key: 'TTA_SOLE_NON_STANDAR_HEADER.-999' //流程唯一标识，在流程管理->流程设计->设计 流程中获取，流程配置时修改为对应表单的流程唯一标识
            };
            ...
            $scope.changeDate = function () {
                if ($scope.info.startTime && $scope.info.endTime) {
                    var year = poorYear($scope.info.startTime, $scope.info.endTime, 1);
                    var year2 = poorYear($scope.info.startTime, $scope.info.endTime, 0);
                    if (year < 1 || year2 >= 3) {
                        $scope.info.isSkipApprove = 'Y';
                    } else {
                        $scope.info.isSkipApprove = 'N';
                    }
                }
            };
            ...
            $scope.submitApprovalEnd = function (){
                ...
                $scope.paramsBpm = {
                    ...
                    "respId": "990021",
                    "processDefinitionKey": "TTA_SOLE_NON_STANDAR_HEADER.-999", //流程唯一标识，需修改为对应业务表单流程唯一标识
                    "saveonly": false,
                    "businessKey": $scope.info.soleNonStandarHeaderId, //单据ID
                    "title": "独家协议(非标准)-审批" + $scope.info.orderNo + "-" + $scope.info.orderVersion, //流程标题，修改为当前业务信息
                    "positionId": 0,
                    "orgId": 0,
                    "userType": "20",
            ...
            $scope.search = function () {
                ...
                httpServer.post(URLAPI.findByIdTtaSoleNonStandarHeader, {
                        'params': JSON.stringify({soleNonStandarHeaderId: getId()})
                    },
                    function (res) {
                        if (res.status == 'S') {
                            ...
                            if (!pdfObject && $scope.info.soleNonStandarHeaderId) {
                                $("#content").append("<iframe id =\"pdfId\" width=\"100%\" height=\"100%\"   src=\""+ $scope.url+"?businessId="+ $scope.info.soleNonStandarHeaderId +"&Certificate=" +Certificate+ "&type=" + "TTA_SOLE_NON_STANDAR_HEADER" +"\"></iframe>");
                            }
                        } else {
                            SIEJS.alert(res.msg, "error", "确定");
                        }
            ...
            $scope.btnSave = function (protocolForm,param) {
                ...
                httpServer.post(URLAPI.saveOrUpdateTtaSoleNonStandarHeader, {
                        'params': JSON.stringify($scope.info)
                    },
                    function (res) {
                        if (res.status == 'S') {
                            if ('1' === param) {
                                $scope.id = res.data.soleNonStandarHeaderId;
                                $scope.search();
                ...
                if (info) {
                    SIEJS.confirm('作废', '是否确定作废？', '确定', function () {
                        ...
                        httpServer.post(URLAPI.cancelTtaSoleNonStandarHeader, {
                            params: JSON.stringify({ids: info.soleNonStandarHeaderId})
                        }, function (res) {
                            if ('S' == res.status) {
                                $scope.search();
                                SIEJS.alert('作废成功');
