...
function GooFlow(bgDiv, property) {
    ...
    this.$nodeDom = {};
    this.$areaDom = {};
    this.$max = property.initNum;
    this.$focus = "";
    this.$cursor = "default";
    this.$editable = property.editable;
    this.$MainContener = property.MainContener;
    var width;
    ...
    var height =  this.$bgDiv.parent().height() || property.height; // 修改　hxz
    ...
    if (property.haveHead) {
        ...
        headHeight = 24;
        this.onBtnNewClick = property.onBtnNewClick;
        this.onBtnOpenClick = property.onBtnOpenClick;
        this.onBtnSaveClick = property.onBtnSaveClick;
        this.onFreshClick = property.onFreshClick;
        this.onExpandClick = property.onExpandClick;
    ...
    if (property.haveTool) {
        HtmlStr = "<div class='GooFlow_tool'" + (property.haveHead ? "" : " style='margin-top:3px'") + "><div style='height:" + (height - headHeight - (property.haveHead ? 7 : 10)) + "px' class='GooFlow_tool_div'></div></div>";
        this.$bgDiv.append(HtmlStr);
        this.$tool = this.$bgDiv.find(".GooFlow_tool div");
        HtmlStr = "<a href='javascript:void(0)' id='" + this.$id + "_btn_cursor' " + (property.toolBtnRemarks.cursor ? "title='" + property.toolBtnRemarks.cursor + "'" : "") + "><i class='fa ico_cursor'/></i><a href='javascript:void(0)' id='" + this.$id + "_btn_direct' " + (property.toolBtnRemarks.direct ? "title='" + property.toolBtnRemarks.direct + "'" : "") + "><i class='fa ico_direct'/></i>";
        this.$tool.append(HtmlStr);
        if (property.toolBtns && property.toolBtns.length > 0) {
            HtmlStr = "<span/>";
            for (var i = 0; i < property.toolBtns.length; i++) {
                HtmlStr += "<a href='javascript:void(0)' id='" + this.$id + "_btn_" + property.toolBtns[i] + "' " + (property.toolBtnRemarks[property.toolBtns[i]] ? "title='" + property.toolBtnRemarks[property.toolBtns[i]] + "'" : "") + "><i class='fa ico_" + property.toolBtns[i] + "'/></i></a>"
            }
            this.$tool.append(HtmlStr)
        }
        if (property.haveGroup) {
            HtmlStr = "<span/><a href='javascript:void(0)' id='" + this.$id + "_btn_group' " + (property.toolBtnRemarks.group ? "title='" + property.toolBtnRemarks.group + "'" : "") + "><i class='fa ico_group'/></i>";
            this.$tool.append(HtmlStr)
        }
        toolWidth = 31;
        this.$nowType = "cursor";
    ...
    this.$workArea = $("<div class='GooFlow_work_inner' style='width:100%;height:100%'></div>").attr({
        unselectable: "on",
...
GooFlow.prototype = {
    useSVG: "", getSvgMarker: function (id, color) {
        ...
        if (this.$editable) {
            $(this.$draw).delegate(tmpClk, "dblclick", {inthis: this}, function (e) {
                ...
                if (This.$lineData[this.id].type == "lr") {
                    ...
                    if (This.$lineData[this.id].type == "tb") {
                ...
                This.$textArea.val(oldTxt).css({
                    ...
                    top: t.top - c.top + y - This.$workArea[0].parentNode.scrollTop
        ...
        this.$group = $("<div class='GooFlow_work_group' style='width:" + workWidth + "px;height:" + workHeight + "px'></div>");
        ...
        this.$group.on("mousedown", {inthis: this}, function (e) {
            ...
            This.$ghost.css({
                ...
                left: This.$areaData[id].left + t.left - c.left - This.$workArea[0].parentNode.scrollLeft + hack + "px",
                cursor: cursor
            });
            var X, Y;
            X = ev.x - t.left + This.$workArea[0].parentNode.scrollLeft;
            ...
            document.onmousemove = function (e) {
                ...
                if (cursor != "move") {
                    ...
                    if (X < t.left - c.left - This.$workArea[0].parentNode.scrollLeft) {
                        ...
                        if (X + This.$workArea[0].parentNode.scrollLeft + This.$areaData[id].width > t.left - c.left + This.$workArea.width()) {
                    ...
                    if (Y < t.top - c.top - This.$workArea[0].parentNode.scrollTop) {
                        ...
                        if (Y + This.$workArea[0].parentNode.scrollTop + This.$areaData[id].height > t.top - c.top + This.$workArea.height()) {
        ...
        if (this.$focus != "") {
            ...
            if (jq.prop("tagName") == "DIV") {
                if (this.onItemBlur != null && !this.onItemBlur(id, "node")) {
                    return false
                }
                jq.removeClass("item_focus").children("div:eq(0)").css("display", "none")
            } else {
                if (this.onItemBlur != null && !this.onItemBlur(id, "line")) {
                    return false
                }
                if (GooFlow.prototype.useSVG != "") {
                    if (!this.$lineData[this.$focus].marked) {
                        jq[0].childNodes[1].setAttribute("stroke", "#ccc");
                        jq[0].childNodes[1].setAttribute("marker-end", "url(#arrow1)")
                    }
                } else {
                    if (!this.$lineData[this.$focus].marked) {
                        jq[0].strokeColor = "#ccc"
                    }
                }
        ...
        if (this.$nodeData[id].type != type) {
            return
        }
        if (this.$nodeData[id].type == newtype) {
            return
        }
        if (this.$nodeData[id].type == "start" || this.$nodeData[id].type == "end") {
            return
        ...
        var This = this;
        $.ajax({
            type: para.type, url: para.url, dataType: "json", data: para.data, success: function (msg) {
                if (para.dataFilter) {
                ...
                if (para.error) {
                    para.error(textStatus, errorThrown)
                }
            }
        })
    }, exportData: function () {
            ...
            if (type == "line") {
                ...
                if (GooFlow.prototype.useSVG != "") {
                    if (mark) {
                        ...
                        this.$nodeDom[id].childNodes[1].setAttribute("stroke", "#ccc");
                        this.$nodeDom[id].childNodes[1].setAttribute("marker-end", "url(#arrow1)")
                    }
                } else {
                    if (mark) {
                        this.$nodeDom[id].strokeColor = "#ff3300"
                    } else {
                        this.$nodeDom[id].strokeColor = "#ccc"
                    }
                }
            }
        }
        ...
        if (this.onItemDel != null && !this.onItemDel(id, "area")) {
            return
        }
        ...
        this.$areaDom[id] = $("<div id='" + id + "' class='GooFlow_area area_" + json.color + "' style='top:" + json.top + "px;left:" + json.left + "px'><div class='bg' style='width:" + (json.width - 2) + "px;height:" + (json.height - 2) + "px'></div><label>" + json.name + "</label><i class='fa' title='点击变换颜色'></i><div><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
...
GooFlow.prototype.mark1 = function (id, type, mark) {
    if (type == "node") {
        if (!this.$nodeData[id]) {
            return
        }
        ...
        this.$nodeData[id].marked = mark || false;
        if (mark) {
            this.$nodeDom[id].addClass("item_mark1")
        } else {
            this.$nodeDom[id].removeClass("item_mark1")
        }
    } else {
        if (type == "line") {
            if (!this.$lineData[id]) {
            ...
            if (GooFlow.prototype.useSVG != "") {
                ...
                jq[0].childNodes[1].setAttribute("stroke", "#393");
                jq[0].childNodes[1].setAttribute("marker-end", "url(#arrow4)")
            } else {
                if (mark) {
                    jq[0].strokeColor = "#393"
                } else {
                    jq[0].strokeColor = "#ccc"
                }
            }
...
GooFlow.prototype.mark2 = function (id, type, mark) {
    if (type == "node") {
        if (!this.$nodeData[id]) {
            return
        }
        ...
        this.$nodeData[id].marked = mark || false;
        if (mark) {
            this.$nodeDom[id].addClass("item_mark2")
        } else {
            this.$nodeDom[id].removeClass("item_mark2")
        }
    } else {
        if (type == "line") {
            if (!this.$lineData[id]) {
...
GooFlow.prototype.addNode = function (id, json) {
    if (this.onItemAdd != null && !this.onItemAdd(id, "node", json)) return;
    if (this.$undoStack) {
        this.pushOper("delNode", [id]);
    }
    var mark = json.mark ? " item_mark" : (json.customMarked ? " item_mark1" : "");
    if (json.type != "start" && json.type != "end") {

        if (json.type == "task" || json.type == "node" || json.type == 'service' || json.type == 'activity') {
            json.width = json.width ? json.width : 80;
            json.height = json.height ? json.height : 80;
            json.top = json.top + json.height / 2 - 40;
            json.left = json.left + json.width / 2 - 40;
        } else {
            json.width = json.width ? json.width : 40;
            json.height = json.height ? json.height : 40;
            json.top = json.top + json.height / 2 - 20;
            json.left = json.left + json.width / 2 - 20;
        }
        ...
        if (json.users) {
            for (var i =0; i < json.users.length; i++) {
                nodeNames.push(json.users[i].personName);
            }
        }

        var nodename =nodeNames.length > 0 ? nodeNames.splice('，') : '';
        var nodeTitle = '';
        if (json.type==='task' && !nodename) {
            nodename = '<span style="color:#ccc">待办理</span>';
            nodeTitle = '待办理'
            if(json.customMarked) {
                nodename = "<span class='user_name' >自动跳过</span>"
                nodeTitle = '自动跳过'
            }
        }
        var nameTitle = json.name.split('<br/>');
        nameTitle = nameTitle.join('，');

        // this.$nodeDom[id] = $("<div class='GooFlow_item" + mark + " " + json.type + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><table cellspacing='1' style='width:" + json.width + "px;height:" + json.height + "px;'><tr><td class='ico'><i class='fa ico_" + /*json.type+*/"'></i></td><td>" + json.name + "</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
        this.$nodeDom[id] = $("<div class='GooFlow_item" + mark + " " + json.type + "' id='" + id + "' style='  top:" + json.top + "px;left:" + json.left + "px'>" +
            "<span class='node_location'></span>" +
            "<div class='GooFlow_item_text'><span class='node_name' title='"+nameTitle+"'><i class='node_arrow'></i>"+json.name+"</span>" +
            "<div class='user_name' data-type='"+json.type +"' title='"+nodeTitle+"'> "+  nodename +"</div></div>" +
            "</div>");
        ...
        if (json.top < 0) json.top = 0;
        if (json.left < 0) json.left = 0;
        this.$nodeDom[id] = $("<div class='GooFlow_item item_round" + mark + " " + json.type + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'>" +
            "<span class='name'>" + json.name + "</span></div>");
    }
    var ua = navigator.userAgent.toLowerCase();
    if (ua.indexOf('msie') != -1 && ua.indexOf('8.0') != -1)
        this.$nodeDom[id].css("filter", "progid:DXImageTransform.Microsoft.Shadow(color=#94AAC2,direction=135,strength=2)");
...
GooFlow.prototype.addLine = function (id, json) {
    ...
    if (!n1 || !n2) return;
    var res;
    var sp = new Array(json.x0, json.y0);
    var ep = new Array(json.x1, json.y1);
    var L = 8;
    if (json.type && json.type != "sl") {
        var m1 = new Array(json.mx0, json.my0);
        var m2 = new Array(json.mx1, json.my1);
        if (n1.type == "start" || n1.type == "fork" || n1.type == "join") {
            //水平或垂直
            if (sp[0] == m1[0]) {//垂直
                sp[1] = sp[1] < m1[1] ? (sp[1] + L > m1[1] ? m1[1] : sp[1] + L) : (sp[1] - L > m1[1] ? sp[1] - L : m1[1]);
            } else if (sp[1] == m1[1]) {//水平
                sp[0] = sp[0] < m1[0] ? (sp[0] + L > m1[0] ? m1[0] : sp[0] + L) : (sp[0] - L > m1[0] ? sp[0] - L : m1[0]);
            } else {
                x_ = parseInt(L / (1 + (sp[1] - m1[1]) * (sp[1] - m1[1]) / ((sp[0] - m1[0]) * (sp[0] - m1[0]))));
                y_ = parseInt(L / (1 + (sp[0] - m1[0]) * (sp[0] - m1[0]) / ((sp[1] - m1[1]) * (sp[1] - m1[1]))));
                sp[0] = sp[0] < m1[0] ? (sp[0] + x_ > m1[0] ? m1[0] : sp[0] + x_) : (sp[0] - x_ > m1[0] ? sp[0] - x_ : m1[0]);
                sp[1] = sp[1] < m1[1] ? (sp[1] + y_ > m1[1] ? m1[1] : sp[1] + y_) : (sp[1] - y_ > m1[1] ? sp[1] - y_ : m1[1]);
            }
        }
        if (n2.type == "end" || n2.type == "fork" || n2.type == "join") {
            //水平或垂直
            if (ep[0] == m2[0]) {//垂直
                ep[1] = ep[1] < m2[1] ? (ep[1] + L > m2[1] ? m2[1] : ep[1] + L) : (ep[1] - L > m2[1] ? ep[1] - L : m2[1]);
            } else if (ep[1] == m2[1]) {//水平
                ep[0] = ep[0] < m2[0] ? (ep[0] + L > m2[0] ? m2[0] : ep[0] + L) : (ep[0] - L > m2[0] ? ep[0] - L : m2[0]);
            } else {
                x_ = parseInt(L / (1 + (ep[1] - m2[1]) * (ep[1] - m2[1]) / ((ep[0] - m2[0]) * (ep[0] - m2[0]))));
                y_ = parseInt(L / (1 + (ep[0] - m2[0]) * (ep[0] - m2[0]) / ((ep[1] - m2[1]) * (ep[1] - m2[1]))));
                ep[0] = ep[0] < m2[0] ? (ep[0] + x_ > m2[0] ? m2[0] : ep[0] + x_) : (ep[0] - x_ > m2[0] ? ep[0] - x_ : m2[0]);
                ep[1] = ep[1] < m2[1] ? (ep[1] + y_ > m2[1] ? m2[1] : ep[1] + y_) : (ep[1] - y_ > m2[1] ? ep[1] - y_ : m2[1]);
            }
        }

        res = {start: sp, m1: m1, m2: m2, end: ep};
        //res=GooFlow.prototype.calcPolyPoints(n1,n2,json.type,json.M);
    } else {
        if (n1.type == "start" || n1.type == "fork" || n1.type == "join") {
            //水平或垂直
            if (sp[0] == ep[0]) {//垂直
                sp[1] = sp[1] < ep[1] ? (sp[1] + L > ep[1] ? ep[1] : sp[1] + L) : (sp[1] - L > ep[1] ? sp[1] - L : ep[1]);
            } else if (sp[1] == ep[1]) {//水平
                sp[0] = sp[0] < ep[0] ? (sp[0] + L > ep[0] ? ep[0] : sp[0] + L) : (sp[0] - L > ep[0] ? sp[0] - L : ep[0]);
            } else {
                x_ = parseInt(L / (1 + (sp[1] - ep[1]) * (sp[1] - ep[1]) / ((sp[0] - ep[0]) * (sp[0] - ep[0]))));
                y_ = parseInt(L / (1 + (sp[0] - ep[0]) * (sp[0] - ep[0]) / ((sp[1] - ep[1]) * (sp[1] - ep[1]))));
                sp[0] = sp[0] < ep[0] ? (sp[0] + x_ > ep[0] ? ep[0] : sp[0] + x_) : (sp[0] - x_ > ep[0] ? sp[0] - x_ : ep[0]);
                sp[1] = sp[1] < ep[1] ? (sp[1] + y_ > ep[1] ? ep[1] : sp[1] + y_) : (sp[1] - y_ > ep[1] ? sp[1] - y_ : ep[1]);
            }
        }
        if (n2.type == "end" || n2.type == "fork" || n2.type == "join") {
            //水平或垂直
            if (ep[0] == sp[0]) {//垂直
                ep[1] = ep[1] < sp[1] ? (ep[1] + L > sp[1] ? sp[1] : ep[1] + L) : (ep[1] - L > sp[1] ? ep[1] - L : sp[1]);
            } else if (ep[1] == sp[1]) {//水平
                ep[0] = ep[0] < sp[0] ? (ep[0] + L > sp[0] ? sp[0] : ep[0] + L) : (ep[0] - L > sp[0] ? ep[0] - L : sp[0]);
            } else {
                x_ = parseInt(L / (1 + (ep[1] - sp[1]) * (ep[1] - sp[1]) / ((ep[0] - sp[0]) * (ep[0] - sp[0]))));
                y_ = parseInt(L / (1 + (ep[0] - sp[0]) * (ep[0] - sp[0]) / ((ep[1] - sp[1]) * (ep[1] - sp[1]))));
                ep[0] = ep[0] < sp[0] ? (ep[0] + x_ > sp[0] ? sp[0] : ep[0] + x_) : (ep[0] - x_ > sp[0] ? ep[0] - x_ : sp[0]);
                ep[1] = ep[1] < sp[1] ? (ep[1] + y_ > sp[1] ? sp[1] : ep[1] + y_) : (ep[1] - y_ > sp[1] ? ep[1] - y_ : sp[1]);
            }
        }
    ...
    this.$draw.appendChild(this.$lineDom[id]);
    if (GooFlow.prototype.useSVG == "") {
        this.$lineDom[id].childNodes[1].innerHTML = json.name ? json.name : '';
        if (this.$lineData[id].type != "sl") {
            var Min = (res.start[0] > res.end[0] ? res.end[0] : res.start[0]);
        ...
        var box = this.$lineDom[id].getBBox();
        // this.$lineDom[id].childNodes[2].textContent=json.name?json.name:'';
        var t = json.name ? json.name : '';
        var odiv = document.createElement('div');
        odiv.style.position = 'absolute';
        odiv.style.top = (box['y']) + 'px';
        odiv.style.left = (box['x']) + 'px';
        odiv.style.width = box['width'] + 'px';
        odiv.style.height = box['height'] + 'px';

        var p = document.createElement('p');
        p.style.position = 'absolute';
        p.style.bottom = '0';
        var tx = document.createTextNode(json.name ? json.name : '');
        p.appendChild(tx);
        odiv.appendChild(p)
        document.getElementsByClassName('GooFlow_work_inner')[0].appendChild(odiv);
    }
    ++this.$lineCount;
}


function processImageDraw(certificate, locationId, processInstanceId, processDefinitionKey, processDefinitionId) {

    // var destine = '//192.168.21.116';//测试环境
    var destine =  window.processServer || '//127.0.0.1';
    ...
    host = domain.indexOf(h) === -1 ? host : destine;

    //var _bpm_process_image_post_url = host + ":2212/bpmServer/bpmProcessService/getGooflowProcessJSON";
    var _bpm_process_image_post_url = host + ":8089/api/bpmServer/bpmProcessService/getGooflowProcessJSON";

    //var _bpm_process_image_post_url = "http://localhost:8050/bpmProcessService/getGooflowProcessJSON";
    //$("body").append("<style>#" + locationId + "{opacity: 0}#" + locationId + " g{cursor: default!important;}</style>");
    var property = {
        haveHead: false,      //是否显示头部工具栏
        haveTool: false,      //是否显示左侧工具栏
        editable: false
    };
    var image = $('#' + locationId).createGooFlow(property);
    window._svgImage = image;
    //添加自定义箭头
    var defs = document.getElementsByTagNameNS("http://www.w3.org/2000/svg", "defs");
    defs[0].appendChild(GooFlow.prototype.getSvgMarker("arrow4", "#393"));
    $.ajax({
        url: _bpm_process_image_post_url,
        type: "post",
        beforeSend: function (xhr) {
            xhr.setRequestHeader("certificate", certificate);
        },
        data: {
        ...
        success: function (res) {
            $('#' + locationId).contents()[0].data = '';
            var data = JSON.parse(res).data;
            image.loadData(data);

            if (data.nodes) {
                for (var nodeId in data.nodes) {
                    if (data.nodes[nodeId].marked) {
                        image.markItem(nodeId, "node", true);
                    } else if (data.nodes[nodeId].customMarked) {
                        image.mark1(nodeId, "node", true);
                    } else if (data.nodes[nodeId].notReceivedMarken) {
                        image.mark2(nodeId, "node", true);
                    }
                }
            }
            if (data.lines) {
                for (var lineId in data.lines) {
                    if (data.lines[lineId].customMarked) {
                        image.mark1(lineId, "line", true);
                    } else if (data.lines[lineId].marked) {
                        image.mark2(lineId, "line", true);
                    }
                }
            }

            //处理svg超出部分连接线被遮挡问题  hemingxing  update:2018年9月17日
            var gItemLeft = 0,
                gItemTop = 0;
            ...
            $('.GooFlow_work_inner').width(parseInt(gItemLeft) + 100).height(parseInt(gItemTop) + 250);
            $('#draw_processImg').width(parseInt(gItemLeft) + 100).height(parseInt(gItemTop) + 250);

            image.onItemFocus = function (id, type) {
                return false;
            }
        }
    });
