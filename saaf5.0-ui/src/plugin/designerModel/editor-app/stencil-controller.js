                ...
                if (shapes && shapes.length > 0) {
                    ...
                    for (var i = 0; i < properties.length; i++) {
                        ...
                        if (propertyConfig === undefined || propertyConfig === null) {
                            ...
                            //add by hemingxing
                            //流程唯一标识不允许修改，并把模型的key赋值到唯一标识上，模型和流程一对一关系
                            if(currentProperty.key === 'oryx-process_id') {
                                currentProperty.value = $scope.modelData.key?$scope.modelData.key:currentProperty.value;
                                currentProperty.readonly = true;
                                currentProperty.mode = 'write';
                            }
                            //流程名称
                            if(currentProperty.key === 'oryx-name'&&!(event.elements.length>0)) {
                                currentProperty.value = $scope.modelData.name;
                                currentProperty.readonly = true;
                                currentProperty.noValue = false;
                                currentProperty.mode = 'write';
                            }
                            //流程作者
                            if(currentProperty.key === 'oryx-process_author') {
                                currentProperty.value = $scope.modelData.key?$scope.modelData.key.split('_')[1]:currentProperty.value;
                                currentProperty.readonly = true;
                                currentProperty.noValue = false;
                            }
